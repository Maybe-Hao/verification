...
import com.hao.test.duanxin.service.RedisService;
import com.hao.test.duanxin.utils.CodeUtil;
import com.hao.test.duanxin.utils.SmsTool;
...
public class SmsCtrlController {
    ...
    /**
     * 前端发送一个json数据，后端用map接受，map里取出电话号码然后发送短信
    ...
    /**
     * 前端发送一个json数据，后端使用map接受，然后查询该手机号对应的验证码，比对是否正确
     * @ResponseBody 返回json数据
     * @RequestMapping 拦截请求，指定请求类型：POST
     * @RequestBody 接受前台传入的json数据 接受类型为Map
     * @throws ClientException 抛出异常
